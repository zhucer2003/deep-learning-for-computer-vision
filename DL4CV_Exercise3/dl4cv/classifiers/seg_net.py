...
class SegNet(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.dec5 = nn.Sequential(*decoders[24:30])
    ...
    def forward(self, x):
        ...
        print(x.size())
        dec1 = self.dec1(x)
        print('dec%d, %s'%(1, dec1.size()))
        dec2 = self.dec2(dec1)
        print('dec%d, %s'%(2, dec2.size()))
        dec3 = self.dec3(dec2)
        print('dec%d, %s'%(3, dec3.size()))
        dec4 = self.dec4(dec3)
        print('dec%d, %s'%(4, dec4.size()))
        dec5 = self.dec5(dec4)
        print('dec%d, %s'%(5, dec5.size()))
        enc5 = self.enc5(dec5)
        print('enc%d, %s'%(5, enc5.size()))
        enc4 = self.enc4(torch.cat([dec4, enc5], 1))
        print('enc%d, %s'%(4, enc4.size()))
        enc3 = self.enc3(torch.cat([dec3, enc4], 1))
        print('enc%d, %s'%(3, enc3.size()))
        enc2 = self.enc2(torch.cat([dec2, enc3], 1))
        print('enc%d, %s'%(2, enc2.size()))
        enc1 = self.enc1(torch.cat([dec1, enc2], 1))
        print('enc%d, %s'%(1, enc1.size()))
